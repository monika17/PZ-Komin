...
{this.t=parent.t;}}},_setDynamicContent:function(n,params,assessment,parent)
{if(!assessment.isDynamicnode||this.h||!n.nodeType||$itxtUtil.ELEMENT_NODE!=n.nodeType||assessment.isSkipnode)
{return;}
var isNewSec=$iTXT.data.Dom.assessDynamicContent(n);if(isNewSec)
{this.v=$iTXT.data.Dom.dynamicContentSections[this._nId];var dcState=$iTXT.data.Context.getDynamicContentState(n);if(!$iTXT.data.Context.changedContentSections[this.v]){$iTXT.data.Context.changedContentSections[this.v]={dynamicContentURL:''};}
if(!params.dynscanwholepage||!dcState)
{}
if(!dcState){this.h=1;$iTXT.data.Context.getDynamicContentURL(this.v);}else{if(!$iTXT.data.Context.changedContentSections[this.v]){var dcURL=$iTXT.glob.params.get('refurl');$iTXT.data.Context.changedContentSections[this.v]={dynamicContentURL:dcURL};if(!$iTXT.data.Context.dynamicContentURLs[dcURL]){$iTXT.data.Context.dynamicContentURLs[dcURL]={};}
...
{var nId=$iTXT.data.Dom.setNodeId(n);if(!VM._.isObject($iTXT.data.Dom.nodeIds[nId])){$iTXT.data.Dom.nodeIds[nId]=this;}
if($iTXT.data.Context.dynamicContentType&$iTXT.data.Context.DC_TYPE_TRANSFORM){var transitionEndEvent='transitionend';var prefixedTransitions={'WebkitTransition':'webkitTransitionEnd','OTransition':'oTransitionEnd','msTransition':'MSTransitionEnd'};for(var t in prefixedTransitions){if(n.style[t]!==undefined){transitionEndEvent=prefixedTransitions[t];break;}}
$iTXT.addEvent(n,transitionEndEvent,function(){$iTXT.data.Context.onContentStateChange(n);});}else if($iTXT.data.Context.dynamicContentType&$iTXT.data.Context.DC_TYPE_MUTATION_OBSERVERS){var obs=new $iTXT.data.Context.mutationObserverHandler(function(mutations){VM._.each(mutations,function(mutation){$iTXT.data.Context.onContentStateChange(n);});});obs.observe(n,{childList:true,subtree:true});}else if($iTXT.data.Context.dynamicContentType&$iTXT.data.Context.DC_TYPE_MUTATION_EVENTS){$iTXT.addEvent(n,'DOMSubtreeModified',function(){$iTXT.data.Context.onContentStateChange(n);});}else{$iTXT.data.Dom.setOnPropChange(n,function(e){$iTXT.data.Dom.onVisibilityChange(n,function(n){$iTXT.data.Context.onContentStateChange(n);});});}}}},_setWeight:function(n,params,assessment,parent)
...
{return-1;}},getDynamicContentState:function(n){var dcState=false;if($iTXT.data.Context.dynamicContentType&$iTXT.data.Context.DC_TYPE_TRANSFORM){dcState=$iTXT.core.Util.getVisiblePercent(n)>$iTXT.data.Context.dynamicContentThreshold;}else if($iTXT.data.Context.dynamicContentType&$iTXT.data.Context.DC_TYPE_MUTATION){dcState=true;}else{dcState=$iTXT.data.Dom.isVisible(n);}
return dcState;},getDynamicContentURL:function(sId){var ccSec=this.changedContentSections[sId],dcURL='';var aURL=$iTXT.glob.params.get('refurl','').split('?',2);var cURL=document.location.href.split('\?')[0];if(aURL!=cURL){dcURL=cURL;}
...
ccSec.dynamicContentURL=dcURL;if(!this.dynamicContentURLs[dcURL]){this.dynamicContentURLs[dcURL]={sent:false};}
return dcURL;},getStartNodes:function()
{if(!this.timings||!$itxtUtil.isObject(this.timings))
{this.timings={};}
...
{if(($iTXT.data.Context.dynamicContentType&$iTXT.data.Context.DC_TYPE_MUTATION)&&$iTXT.data.Context.dynScanned){return;}
$iTXT.data.Context.dynScanned=true;var nId=$iTXT.data.Dom.setNodeId(n);var cN=$iTXT.data.Dom.nodeIds[nId];var dcSec=cN.v;if(!VM._.isObject(cN)){return;}else if(!dcSec){return;}
var oParams=this.params.get('oparams');this.timings.cStart=new Date().getTime();this.harvestCount++;this.modifiedTrees[this.modifiedTrees.length]={nodeId:nId,fieldName:'v',fieldValue:cN.v,newTree:new this.Node(n,oParams).simplify()};this.timings.cEnd=new Date().getTime();var dcURL=this.getDynamicContentURL(dcSec);if(!this.dynamicContentURLs[dcURL]){this.dynamicContentURLs[dcURL]={sent:false};}
var doneFlag=false;if(!($iTXT.data.Context.dynamicContentType&$iTXT.data.Context.DC_TYPE_MUTATION)){doneFlag=this.dynamicContentURLs[dcURL].sent;}
var dcState=$iTXT.data.Context.getDynamicContentState(n);if(dcState){$iTXT.glob.params.set('refurl',dcURL,100);var spliced=this.modifyTree();this.lastTime=(this.timings.cEnd-this.timings.cStart);if($iTXT.debug.Util.isLoggingOn())
...
this.dynamicContentURLs[dcURL].sent=true;}},preprocessExcludeNodes:function(params)
...
{var vN=this.params.get('dynamicnodes',[]);this.dynamicContentMode=!!(vN.length>0);if(this.dynamicContentMode){var resetFlag=function(){window.setTimeout(function(){$iTXT.data.Context.dynScanned=false;},1000);}
$iTXT.subscribe("$iTXT:initialiser:load:complete",resetFlag);$iTXT.subscribe("$iTXT:hooks:loaded",resetFlag);}},setDynamicContentThreshold:function()
{this.dynamicContentThreshold=this.params.get('dynamicthreshold',15);},setDynamicContentType:function()
{var dcType=this.params.get('dynamictype','visibility');if('transform'==dcType){this.dynamicContentLabel='CSS Transformation';this.dynamicContentType=$iTXT.data.Context.DC_TYPE_TRANSFORM;}else if('mutation'==dcType){if('undefined'!=typeof WebKitMutationObserver){this.dynamicContentLabel='WebKit Mutation Observer';this.mutationObserverHandler=WebKitMutationObserver;this.dynamicContentType=$iTXT.data.Context.DC_TYPE_MUTATION+$iTXT.data.Context.DC_TYPE_MUTATION_OBSERVERS;}else if('undefined'!=typeof MutationObserver){this.dynamicContentLabel='Mutation Observer';this.mutationObserverHandler=MutationObserver;this.dynamicContentType=$iTXT.data.Context.DC_TYPE_MUTATION+$iTXT.data.Context.DC_TYPE_MUTATION_OBSERVERS;}else if('undefined'!=typeof MutationEvent){this.dynamicContentLabel='Mutation Events';this.dynamicContentType=$iTXT.data.Context.DC_TYPE_MUTATION+$iTXT.data.Context.DC_TYPE_MUTATION_EVENTS;}else{this.dynamicContentMode=false;this.dynamicContentType=0;this.dynamicContentLabel='';}}else{this.dynamicContentLabel='Visibility';this.dynamicContentType=this.DC_TYPE_VISIBILITY;}
if(this.dynamicContentLabel){}},setTreeObjectMode:function()
...
var dcState=$iTXT.data.Context.getDynamicContentState(elmt);if(pState==null||pState!=dcState)
{if(null==tSec)
...
{var nId=this.setNodeId(elmt);var dynSec=this.dynamicContentSections[nId];if(undefined!=dynSec)
{var currState=this.isVisible(elmt);var prevState=this.dynamicContentStates[dynSec];if(prevState!=currState)
{this.dynamicContentStates[dynSec]=currState;f(elmt);return true;}}
